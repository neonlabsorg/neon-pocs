...
contract MemeLaunchpad is ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        require(_feePercent <= 500, "ERROR: ADMIN RISK"); // token sale fee cannot be bigger than 5%
    ...
    event Test(uint64 amount0, uint64 amount1, uint64 amount2);
    ...
    function buy(
        ...
        require(tokens[tokenAddress].state == TokenState.FUNDING, "Token not found");
        ...
        uint64 contributionWithoutFee = wsolAmount - ((wsolAmount * feePercent) / FEE_DENOMINATOR);
        if (contributionWithoutFee > leftToFundingGoal) {
            wsolAmount = leftToFundingGoal + (leftToFundingGoal * feePercent) / FEE_DENOMINATOR;
            contributionWithoutFee = leftToFundingGoal;
        }
        uint64 _fee = wsolAmount - contributionWithoutFee;
        fee += _fee;
        
        wsolToken.transferFrom(msg.sender, address(this), wsolAmount);
